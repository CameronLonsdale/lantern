...
def _test_simplesubstitution(plaintext, *fitness_functions, key=None, ntrials=30, top_n=1):
    if key is None:
        key = list(string.ascii_uppercase)
        random.shuffle(key)

    ciphertext = pycipher.SimpleSubstitution(key).encipher(plaintext, keep_punct=True)
    decryptions = simplesubstitution.crack(ciphertext, *fitness_functions, ntrials=ntrials)
